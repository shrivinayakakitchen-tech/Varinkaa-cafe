...
const QuantityButton: React.FC<{ onClick: () => void; children: React.ReactNode; disabled?: boolean }> = ({ onClick, children, disabled }) => (
  <button 
    onClick={onClick} 
    disabled={disabled}
    className="w-7 h-7 flex items-center justify-center font-bold text-lg bg-gray-100 rounded-full text-blue-700 hover:bg-gray-200 transition-transform duration-150 active:scale-90 transform disabled:opacity-50 disabled:cursor-not-allowed"
  >
    {children}
  </button>
);
...
export const Cart: React.FC<CartProps> = ({ isOpen, onClose, selectedItems, onUpdateItem, whatsAppUrl }) => {
  ...
  return (
    <div className={`fixed inset-0 z-50 flex justify-end transition-all duration-300 ease-in-out ${isOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`} role="dialog" aria-modal="true" aria-labelledby="cart-heading">
      {/* Overlay */}
      <div className="absolute inset-0 bg-black/50" onClick={onClose} aria-hidden="true"></div>

      {/* Cart Panel */}
      <div className={`relative w-full max-w-md bg-white flex flex-col transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        {/* Header */}
        <div className="flex items-center justify-between p-4 border-b">
          <h2 id="cart-heading" className="text-xl font-bold text-slate-800">Your Order</h2>
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {orderItems.length === 0 ? (
            <div className="text-center py-10 text-slate-500">
              ...
              <h3 className="mt-2 text-lg font-semibold text-slate-800">Your cart is empty</h3>
              <p className="mt-1 text-sm text-slate-600">Add some delicious items from the menu!</p>
            </div>
          ) : (
        ...
        {orderItems.length > 0 && (
          <div className="p-4 border-t bg-gray-50">
            ...
            <a
              ...
              className="w-full flex items-center justify-center gap-3 px-4 py-3 rounded-lg no-underline font-extrabold bg-green-500 hover:bg-green-600 text-white shadow-lg transition-transform transform hover:scale-105 active:scale-95"
            >
